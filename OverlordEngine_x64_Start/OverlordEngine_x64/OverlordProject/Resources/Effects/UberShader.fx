...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    {
        output = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord) * pow(saturate(dot(reflect(gLightDirection, -normal), viewDirection)), gShininess);
    }
    else
    {
        output = gColorSpecular * pow(saturate(dot(reflect(gLightDirection, -normal), normalize(viewDirection + gLightDirection))), gShininess);
    }
    return output;
}
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    {
        output = gColorSpecular * pow(saturate(dot(reflect(gLightDirection, -normal), viewDirection)), gShininess);
    }
    return output;
}
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if (gUseTextureNormal)
    {
        ...
        float3 biNormal = mul(tangent, normalSample);
		if (gFlipGreenChannel)
        {
            biNormal = -biNormal;
        }
        newNormal = normalize(biNormal);
    }
	return newNormal;
}
...
{
    float3 envMapSample = float3(0, 0, 0);
	if(gUseEnvironmentMapping)
    {
        float3 reflected = reflect(viewDirection, normal);
        float3 refracted = refract(viewDirection, normal, gRefractionIndex) * gRefractionStrength;

        float3 reflectedSample = gCubeEnvironment.Sample(gTextureSampler, reflected);
        float3 refractedSample = gCubeEnvironment.Sample(gTextureSampler, refracted);
        envMapSample = reflectedSample * gReflectionStrength + refractedSample * gRefractionStrength;
